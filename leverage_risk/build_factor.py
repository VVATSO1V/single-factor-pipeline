"""Build the point-in-time leverage risk factor.

    factor(T) = -total_liabilities / total_assets

Higher values therefore represent lower balance-sheet leverage.
...
PIT_FIELDS = ["total_liabilities", "total_assets"]
HISTORY_QUARTERS = 4
...
def compute_value(state: dict[int, pd.Series], current_quarter: int) -> float:
    ...
    if not np.isfinite(liabilities) or not np.isfinite(assets) or assets <= 0:
        return np.nan
    return float(-liabilities / assets)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build leverage_risk factor CSV.")
