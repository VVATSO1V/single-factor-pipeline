"""Build factor.csv for the 1-day reversal factor.

The factor is:

    rev_1d(T) = -r_T = -(P_T / P_{T-1} - 1)
"""
...
def build_factor(close: pd.DataFrame, start_date: str) -> pd.DataFrame:
    factor = -(close / close.shift(1) - 1.0)
    return long_factor_from_wide(factor, start_date)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build rev_1d factor CSV.")
...
def main() -> None:
    ...
    price_start = lookback_start_date(rq, args.start_date, 2)
