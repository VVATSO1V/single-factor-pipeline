"""Build factor.csv for the skip-1 20-day reversal factor.

The factor is:

    rev_20d_skip1(T) = -(P_{T-1} / P_{T-21} - 1)

The current day's close is intentionally skipped.
"""
...
def build_factor(close: pd.DataFrame, start_date: str) -> pd.DataFrame:
    factor = -(close.shift(1) / close.shift(21) - 1.0)
    return long_factor_from_wide(factor, start_date)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build rev_20d_skip1 factor CSV.")
...
def main() -> None:
    ...
    price_start = lookback_start_date(rq, args.start_date, 22)
