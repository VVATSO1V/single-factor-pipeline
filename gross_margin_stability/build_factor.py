"""Build the point-in-time gross margin stability factor.

    factor(T) = -std(last 8 announced single-quarter gross margins)

Chinese income statements are year-to-date, so Q2-Q4 single-quarter values
are obtained by subtracting the preceding report in the same fiscal year.
...
PIT_FIELDS = ["gross_profit", "operating_revenue"]
HISTORY_QUARTERS = 12
...
def compute_value(state: dict[int, pd.Series], current_quarter: int) -> float:
    margins = []
    for quarter in range(current_quarter - 7, current_quarter + 1):
        row = state.get(quarter)
        if row is None:
            return np.nan
        gross_profit = row["gross_profit"]
        revenue = row["operating_revenue"]
        if quarter % 4 != 0:
            previous = state.get(quarter - 1)
            if previous is None:
                return np.nan
            gross_profit -= previous["gross_profit"]
            revenue -= previous["operating_revenue"]
        if not np.isfinite(gross_profit) or not np.isfinite(revenue) or revenue <= 0:
            return np.nan
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build gross_margin_stability factor CSV.")
