"""Build factor.csv for the 5-day overnight reversal factor.

The factor is:

    overnight_rev_5d(T) = -mean(O_i / C_{i-1} - 1), i=T-4...T
"""
...
def build_factor(price: pd.DataFrame, window: int, start_date: str) -> pd.DataFrame:
    ...
    overnight_return = open_price / close_price.shift(1) - 1.0
    factor = -overnight_return.rolling(window=window, min_periods=window).mean()
    return long_factor_from_wide(factor, start_date)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build overnight_rev_5d factor CSV.")
