"""Build the point-in-time cash earnings quality factor.

    factor(T) = operating_cash_flow_ttm / net_profit_ttm

Statements become visible on the trading date after their announcement.
Observations with
non-positive net profit are excluded because the ratio loses its quality
interpretation for loss-making companies.
"""
...
PIT_FIELDS = ["net_operate_cashflowTTM", "net_profitTTM"]
HISTORY_QUARTERS = 4
...
def compute_value(state: dict[int, pd.Series], current_quarter: int) -> float:
    ...
    net_profit = row["net_profitTTM"]
    cash_flow = row["net_operate_cashflowTTM"]
    if not np.isfinite(net_profit) or not np.isfinite(cash_flow) or net_profit <= 0:
        return np.nan
    return float(cash_flow / net_profit)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build cash_earnings_quality factor CSV.")
