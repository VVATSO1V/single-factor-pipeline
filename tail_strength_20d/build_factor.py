"""Build factor.csv for the 20-day tail strength factor.

The factor is:

    tail_strength_20d(T) = mean((C_i - O_i) / (H_i - L_i)), i=T-19...T
"""
...
def build_factor(price: pd.DataFrame, window: int, start_date: str) -> pd.DataFrame:
    ...
    high_price = wide_field(price, "high")
    low_price = wide_field(price, "low")
    daily_tail = (close_price - open_price) / (high_price - low_price)
    daily_tail = daily_tail.where((high_price - low_price) > 0)
    factor = daily_tail.rolling(window=window, min_periods=window).mean()
    return long_factor_from_wide(factor, start_date)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build tail_strength_20d factor CSV.")
...
def main() -> None:
    ...
    price_start = lookback_start_date(rq, args.start_date, args.window + 1)
    ...
    price = fetch_ohlc(rq, stocks, price_start, args.end_date, ["open", "close", "high", "low"])
